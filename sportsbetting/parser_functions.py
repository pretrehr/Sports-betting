...
def parse_pasinobet_sport2(sport):
    #     selenium_init.start_selenium("pasinobet", False)
    selenium_init.DRIVER["pasinobet"].get(
        "https://www.pasinobet.fr/#/sport/?type=0")
    promotion = WebDriverWait(selenium_init.DRIVER["pasinobet"], 15).until(
        EC.element_to_be_clickable(
            (By.XPATH, "//button[@ng-click='closeDialog(activeDialog, 'okButton');$event.stopPropagation();']"))
    )
    promotion.click()
    cookies = WebDriverWait(selenium_init.DRIVER["pasinobet"], 15).until(
        EC.element_to_be_clickable(
            (By.XPATH, "//button[@ng-click='acceptCookies()']"))
    )
    cookies.click()
    ...
    football = WebDriverWait(selenium_init.DRIVER["pasinobet"], 15).until(
        EC.element_to_be_clickable(
            (By.XPATH, "//div[@class='sports-item-v3 Soccer active']"))
    )
    football.click()
    sport_element = WebDriverWait(selenium_init.DRIVER["pasinobet"], 15).until(
        EC.element_to_be_clickable(
            (By.XPATH, "//div[@class='sports-item-v3 {}']".format(sport)))
    )
    sport_element.click()
    regions = WebDriverWait(selenium_init.DRIVER["pasinobet"], 15).until(
        EC.presence_of_all_elements_located(
            (By.XPATH, "//div[@class='region-item-v3 ng-binding']"))
    )
    for region in regions[::-1]:
        try:
            ...
            selenium_init.DRIVER["pasinobet"].execute_script(
                'document.getElementsByClassName("full-menu-wrapper-box-v3")[0].scroll(0, -500)')
            region.click()
    ...
    for competition in competitions:
        try:
            ...
            WebDriverWait(selenium_init.DRIVER["pasinobet"], 15).until(
                EC.presence_of_all_elements_located(
                    (By.CLASS_NAME, "team-name-view-table-v3"))
            )
            inner_html = selenium_init.DRIVER["pasinobet"].execute_script(
                "return document.body.innerHTML")
            odds_page = parse_pasinobet_html(
                inner_html, sport == "Basketball", "NBA" in competition.text)
...
def parse_pasinobet_html(inner_html, is_basketball, is_us):
    ...
    if is_basketball:
        ...
        links = []
        for _ in range(100):
            links = selenium_init.DRIVER["pasinobet"].find_elements_by_class_name(
                'team-name-tc')
            if links:
                break
        for match_link in links:
            ...
            for line in soup.findAll():
                if "data-title" in line.attrs and "Vainqueur du match" in line["data-title"]:
                    odds = list(
                        map(float, list(line.find_parent().stripped_strings)[1::2]))
